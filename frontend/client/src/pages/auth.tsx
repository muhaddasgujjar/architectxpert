...
import { Hexagon, Eye, EyeOff, ArrowRight, ArrowLeft, User, Lock, Mail, CheckCircle2 } from "lucide-react";
import { useLocation } from "wouter";
import { useAuth } from "@/hooks/use-auth";
import { use3DTilt, fadeUp } from "@/lib/animations";

function AnimatedInput({
  type, value, onChange, placeholder, icon: Icon, suffix, autoComplete,
}: {
  type: string; value: string; onChange: (v: string) => void;
  placeholder: string; icon: React.ElementType; suffix?: React.ReactNode; autoComplete?: string;
}) {
  const [focused, setFocused] = useState(false);

  return (
    <div className="relative">
      <motion.div animate={{ opacity: focused ? 0.8 : 0.3 }} transition={{ duration: 0.2 }}>
...
type AuthMode = "login" | "register" | "forgot" | "reset-success";
                ...
                >
                  {successMsg ? (
                    <div className="text-center py-4">
                      ...
                      <p className="text-white/70 text-sm mb-6">{successMsg}</p>
                      <button
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                      ...
                      <motion.button
                        ...
                        className="w-full flex items-center justify-center gap-2 bg-accent-blue hover:bg-blue-500 text-white py-3.5 rounded-xl text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                ...
                >
                  {mode === "register" && (
                    <motion.div variants={fadeUp} initial="hidden" animate="show">
                      <label className="block text-xs font-mono text-white/40 mb-2 uppercase tracking-wider">Full Name</label>
                      <AnimatedInput
                        type="text"
                        value={fullName}
                  ...
                  <AnimatePresence mode="wait">
                    {mode === "register" && (
                      ...
                      >
                        <label className="block text-xs font-mono text-white/40 mb-2 uppercase tracking-wider">Confirm Password</label>
                        <AnimatedInput
                          type={showPassword ? "text" : "password"}
